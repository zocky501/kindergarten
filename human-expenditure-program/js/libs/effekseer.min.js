...
{var arrayBuffer=Module.resourcesMap[path];if(arrayBuffer!=null){var arrayBufferView=new Uint8Array(arrayBuffer);Promise.resolve(new Blob([arrayBufferView],{type:'image/png'})).then(function(blob){return Promise.resolve(URL.createObjectURL(blob));}).then(function(url){var img=new Image();img.crossOrigin = 'anonymous';img.onload=function(){res.image=img;res.isLoaded=true;effect._update();};img.src=url;});}else{_loadResource(path,function(image){res.image=image;res.isLoaded=true;effect._update();},effect.onerror);}}
